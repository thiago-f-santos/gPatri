...
public class UsuarioService {
    ...
    public UserResponseDTO criarUsuario(UserRequestDTO userRequestDTO) {
        ...
        Cargo cargo = cargoRepository.findByNome(CargoService.CARGO_USUARIO)
                .orElseThrow(() -> new ResourceNotFoundException(String.format("Cargo '%s' não encontrado.", CargoService.CARGO_USUARIO)));
    ...
    public UserResponseDTO atribuirCargo(UUID id, UsuarioCargoUpdateDTO usuarioCargoUpdateDTO) {
        ...
        if (usuario.getCargo() != null && usuario.getCargo().getId().equals(cargo.getId())) {
            throw new BadRequestException("O usuário já possui este cargo atribuído.");
        }
